var createMediapipeSolutionsWasm = (() => {
	...
	return function (createMediapipeSolutionsWasm) {
		...
		function _wgpuDeviceCreateBindGroup(deviceId, descriptor) {
			...
			function makeEntry(entryPtr) {
				...
				if (bufferId) {
					...
					return {
						binding: binding,
						resource: { buffer: WebGPU.mgrBuffer.get(bufferId), offset: HEAPU32[(entryPtr + 4 + 16) >> 2] * 4294967296 + HEAPU32[(entryPtr + 16) >> 2], size: size },
					}
				} else if (samplerId) {
		...
		function _wgpuDeviceCreateComputePipeline(deviceId, descriptor) {
			assert(descriptor)
			assert(HEAPU32[descriptor >> 2] === 0)
			var desc = { label: undefined, layout: WebGPU.mgrPipelineLayout.get(HEAPU32[(descriptor + 8) >> 2]), compute: WebGPU.makeProgrammableStageDescriptor(descriptor + 12) }
			var labelPtr = HEAPU32[(descriptor + 4) >> 2]
			if (labelPtr) desc["label"] = UTF8ToString(labelPtr)
			var device = WebGPU.mgrDevice.get(deviceId)
		...
		function _wgpuDeviceCreateRenderPipeline(deviceId, descriptor) {
			...
			function makeBlendComponent(bdPtr) {
				...
				return { operation: WebGPU.BlendOperation[HEAPU32[bdPtr >> 2]], srcFactor: WebGPU.BlendFactor[HEAPU32[(bdPtr + 4) >> 2]], dstFactor: WebGPU.BlendFactor[HEAPU32[(bdPtr + 8) >> 2]] }
			}
			...
			function makeColorState(csPtr) {
				...
				return formatInt === 0 ? undefined : { format: WebGPU.TextureFormat[formatInt], blend: makeBlendState(HEAPU32[(csPtr + 8) >> 2]), writeMask: HEAPU32[(csPtr + 12) >> 2] }
			}
			...
			function makeVertexAttribute(vaPtr) {
				assert(vaPtr)
				return { format: WebGPU.VertexFormat[HEAPU32[vaPtr >> 2]], offset: HEAPU32[(vaPtr + 4 + 8) >> 2] * 4294967296 + HEAPU32[(vaPtr + 8) >> 2], shaderLocation: HEAPU32[(vaPtr + 16) >> 2] }
			}
			...
			function makeMultisampleState(msPtr) {
				...
				return { count: HEAPU32[(msPtr + 4) >> 2], mask: HEAPU32[(msPtr + 8) >> 2], alphaToCoverageEnabled: HEAP8[(msPtr + 12) >> 0] !== 0 }
			}
		...
		var ___start_em_js = (Module["___start_em_js"] = 711795)
		var ___stop_em_js = (Module["___stop_em_js"] = 712315)
